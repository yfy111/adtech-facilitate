...
public class SetNullToStringUtil {
    ...
    public <T> void setValue(T t, String rep) {
        try {
            ...
            //获取参数名字
            for (Field f : fields) {
                Object o = InvokeUtil.invokeGet(f.getName(), t);
...
//                    property = f.getAnnotation(DesensitizationProperty.class);
                    ...
                    if (property != null && !StringUtils.isEmpty(property.rule())) {
                        if (CommonVariable.DECTIONALRY_CAST.equals(property.doSomeThing())) {
                            ...
                            //对应数据
                            if (!StringUtils.isEmpty(property.loadTableName())) {
                                if (CommonVariable.LOAD_TABLE_NAME_REGION.equals(property.loadTableName())) {
                                    //编码补全
                            ...
                            if (value == null) {
                                //刷新字典
//                                log.info("刷新redis开始时间："+System.currentTimeMillis());
                                this.reloadRedis(property.loadTableName(), property.rule(), o.toString());
//                                log.info("刷新redis结束时间："+System.currentTimeMillis());
                                //加载后重新赋值
...
//                                if(value==null)
                            ...
                            InvokeUtil.invokeSet(f.getName(), value, f, t);
                            o = value;
                        } else if (property.doSomeThing().equals(CommonVariable.DESENSITIZATION)) {
                            if (!"null".equals(o) &&!StringUtils.isEmpty(o.toString().trim())) {
                                ...
                                InvokeUtil.invokeSet(f.getName(), o, f, t);
                            }
                        }
                    }
                }
                //空值转换
                if (StringUtils.isEmpty(o) || "null".equals(o) || StringUtils.isEmpty(o.toString().trim())) {
                    InvokeUtil.invokeSet(f.getName(), rep, f, t);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
...
/**
    ...
    private void reloadRedis(String dTable, String key, String code) {
        //医院刷新
